...
module.exports = {
  ...
  classifyPatients: async (vitals, patientId) => {
    ...
    let tempVariance =
      Math.pow(temperature - normalVals.temperature, 2) /
      normalVals.temperature;
    let heartRateVariance =
      Math.pow(heartRate - normalVals.heartRate, 2) / normalVals.heartRate;
    let oxygenVariance =
      Math.pow(SpO2 - normalVals.oxygen, 2) / normalVals.oxygen;
    let chiVal = Math.pow(
      tempVariance + heartRateVariance + oxygenVariance,
      0.5
    );
